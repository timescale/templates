with m as ( select * from stats_agg_1m_sample),
h as ( select * from stats_agg_1h_sample),
d as ( select * from stats_agg_1d_sample),
mon as ( select * from stats_agg_monthly_sample)
select
  m.bucket as time,
  m.device_id,
...
from m
  join   h on time_bucket('1h', m.bucket) = h.bucket and m.device_id = h.device_id
